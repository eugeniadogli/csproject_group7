...
class CustomerDashboard(tk.Frame):
    def __init__(self,main,custname):  #tk Frame acts as the parent class for AdminDashboard, inheritting it's functions!
        self.main = main
        self.custname = custname
        #use of self.main because main is or window dashboard

        self.main.title(f"Gridcare-Lite - Hello, Customer {self.custname}")
        ...
        self.heading = ttk.Label(
            self.main_frame, text=f"Welcome Customer {self.custname}", font=('Helvetica', 16), background="#36c0ad", foreground="white")
        self.heading.pack()
'''
main = tk.Tk()
dashboard = CustomerDashboard(main, "Eugenia") #ok, username has to be gotten from the login window but hard coded for now/
